...
#        self.longitudes = np.unique(pcr2numpy(pcr.xcoordinate(cloneMap), vos.MV))
        self.latitudes = pcr2numpy(pcr.ycoordinate(cloneMap), vos.MV)
        self.longitudes = pcr2numpy(pcr.xcoordinate(cloneMap), vos.MV)
...
#        rootgrp.createDimension('lon',len(self.longitudes))
        rootgrp.createDimension('lat',self.latitudes.shape)
        rootgrp.createDimension('lon',self.longitudes.shape)

        date_time= rootgrp.createVariable('time','f4',('time',))
        date_time.standard_name= 'time'
        date_time.long_name= 'Hours since 1970-01-01'

        date_time.units= 'Hours since 1970-01-01'
