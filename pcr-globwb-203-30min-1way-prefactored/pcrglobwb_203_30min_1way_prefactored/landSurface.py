...
class LandSurface(object):
    ...
    def __init__(self,iniItems,landmask,initialState=None):
        ...
        # ADDED: include new floodplainWaterLayer variable for proper WB checks of 2-way coupled model
        # ----------------------------------------------------------------------------------------------------------------
        if self.ActivateCoupling == 'True':
            self.stateVars += ['floodplainWaterLayer']
        # ----------------------------------------------------------------------------------------------------------------
    ...
    def update(self,meteo,groundwater,routing,currTimeStep):
        ...
        for coverType in self.coverTypes:
            ...
            for var in self.aggrVars:
                # ADDED/CHANGED: include new floodplainWaterLayer variable for proper WB checks of 2-way coupled model
                # ----------------------------------------------------------------------------------------------------------------
                if self.ActivateCoupling == 'True':
                    if var == 'floodplainWaterLayer':
                        vars(self)[var] += \
                             self.landCoverObj[coverType].fracVegCover*\
                             vars(self.landCoverObj[coverType])[var]*\
                             self.landCoverObj[coverType].inundatedFraction
                    else:
                        vars(self)[var] += \
                             self.landCoverObj[coverType].fracVegCover*\
                             vars(self.landCoverObj[coverType])[var]
