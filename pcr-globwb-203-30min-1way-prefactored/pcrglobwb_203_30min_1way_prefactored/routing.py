...
class Routing(object):
    ...
    def __init__(self,iniItems,initialConditions,lddMap):
        ...
        self.preventRunoffToDischarge = pcr.ifthen(self.landmask, pcr.scalar(1.0))    # to control runoff being added to discharge
        self.controlDynamicFracWat    = pcr.ifthen(self.landmask, pcr.scalar(1.0))    # to control the fraction of open water
        self.waterBodyFractionFM      = pcr.ifthen(self.landmask, pcr.scalar(0.0))    # fraction of open water from D-Flow FM
        # ----------------------------------------------------------------------------------------------------------------
    ...
    def update(self,landSurface,groundwater,currTimeStep,meteo):
        ...
        # ADDED: new calculation for dynamicFracWat in case coupling is activated (should update coupled cells only)
        #-----------------------------------------------------------------------------------------------------
        if self.ActivateCoupling == 'True':
            self.dynamicFracWat = (self.dynamicFracWat * self.controlDynamicFracWat) + self.waterBodyFractionFM
        #-----------------------------------------------------------------------------------------------------
    ...
    def simple_update(self,landSurface,groundwater,currTimeStep,meteo):
        ...
        # CHANGED: added a variable controlling where landSurfaceRunoff should be added to total and where not
        #-----------------------------------------------------------------------------------------------------
        if self.ActivateCoupling == 'True':
            self.runoff = (landSurface.landSurfaceRunoff * self.preventRunoffToDischarge) +\
