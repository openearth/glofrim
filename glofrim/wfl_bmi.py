...
class WFL(GBmi):
    ...
    def initialize_config(self, config_fn):
        ...
        self._bmi.initialize_config(
            str(self._config_fn), loglevel=self._loglevel)
    ...
    def update(self, dt=None):
        ...
        if (dt is not None) and (dt != self._dt.total_seconds()):
            ...
            tt = timedelta(0)
            # loop required ad self._bmi.update(dt) does not work
            while tt < dt:
                self._bmi.update()
                tt += self._dt
        else:
    ...
    def get_value(self, long_var_name, fill_value=-999, **kwargs):
        ...
        var = np.asarray(self._bmi.get_value(long_var_name))[
            ::-1, :]  # flip array back to N->S
    ...
    def get_grid(self, mapdir=None):
        if not hasattr(self, 'grid') or (self.grid is None):
            ...
            if not isfile(_ldd_fn):
                raise IOError('ldd file not found')
            # landmask used for masking out coordinates outside mask
            _lm_fn = join(mapdir, 'wflow_subcatch.map')
            if not isfile(_lm_fn):
                raise IOError('subcatch file not found')
            self.logger.info(
                'Getting rgrid info based on {}'.format(basename(_lm_fn)))
            with rasterio.open(_lm_fn, 'r') as ds:
                ...
                self.grid = RGrid(ds.transform, ds.height,
                                  ds.width, crs=ds.crs, mask=ds.read(1) >= 1)
