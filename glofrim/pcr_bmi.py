...
class PCR(GBmi):
    ...
    def get_start_time(self):
        ...
        if self.initialized:
            ...
            startTime = datetime.combine(
                self._bmi.get_start_time(), datetime.min.time())
        else:
    ...
    def get_end_time(self):
        ...
        if self.initialized:
            ...
            endTime = datetime.combine(
                self._bmi.get_end_time(), datetime.min.time())
        else:
    ...
    def get_value(self, long_var_name, fill_value=-999, **kwargs):
        ...
        var = np.asarray(self._bmi.get_var(
            long_var_name, missingValues=fill_value, **kwargs))
    ...
    def set_value(self, long_var_name, src, fill_value=-999, **kwargs):
        ...
        src = np.where(np.isnan(src), fill_value, src).astype(
            self.get_var_type(long_var_name))
        self._bmi.set_var(long_var_name, src,
                          missingValues=fill_value, **kwargs)
    ...
    def _get_tot_volume_in(self):
        # precipitation [m]
        P = np.nansum(self.get_value('precipitation')
                      * self.get_value('cellArea'))
        return P
    ...
    def get_grid(self):
        ...
        if not hasattr(self, 'grid') or (self.grid is None):
            ...
            _indir = abspath(self.get_attribute_value(
                'globalOptions:inputDir'))
            _ldd_fn = glib.getabspath(self.get_attribute_value(
                'routingOptions:lddMap'), _indir)
            ...
            _lm_fn = glib.getabspath(self.get_attribute_value(
                'globalOptions:landmask'), _indir)
            if isfile(_lm_fn):
            ...
            self.logger.info('Getting rgrid info based on {}; mask based on {}'.format(
                basename(_ldd_fn), mask_name))
            with rasterio.open(_ldd_fn, 'r') as ds:
                if mask is None:
                    mask = ds.read(1) != ds.nodata
                self.grid = RGrid(ds.transform, ds.height,
                                  ds.width, crs=ds.crs, mask=mask)
            # read file with pcr readmap
            self.logger.info(
                'Getting drainage direction from {}'.format(basename(_ldd_fn)))
