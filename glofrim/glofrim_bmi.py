...
class Glofrim(EBmi):
    ...
    # need to initialize before we can know the grid
    ...
    def __init__(self, loglevel=logging.INFO):
        ...
        self.wb_logger = setlogger(
            None, 'wb', thelevel=loglevel, show_in_console=False)
    ...
    def _check_long_var_name(self, long_var_name):
        ...
        if not (len(long_var_name.split(self._var_sep)) == 2):
            msg = 'use "model{}var" syntax for long_var_name'.format(
                self._var_sep)
    ...
    def _check_initialized(self):
        ...
        self.initialized = np.all(
            [self.bmimodels[mod].initialized for mod in self.bmimodels])
    ...
    def initialize_config(self, config_fn, env_fn=None):
        ...
        # log to file
        add_file_handler(self.logger, abspath(
            config_fn).replace('.ini', '.log'))
        if self.initialized:
        ...
        if env_fn is not None and isfile(abspath(env_fn)):
            ...
            for sect in env.sections():
                ...
                for opt in env.options(sect):
                    self._config.set(sect, opt, glib.getabspath(
                        env.get(sect, opt), dirname(env_fn)))
        ...
        for mod in self._config.options('models'):
            ...
            if 'engine' in _bmi.__init__.__code__.co_varnames:
                ...
                engine_path = glib.getabspath(
                    self._config.get('engines', mod), self._root)
            ...
            modconf = glib.getabspath(
                self._config.get('models', mod), self._root)
        ...
        add_file_handler(self.wb_logger, abspath(config_fn).replace(
            '.ini', '.wb'), formatter=logging.Formatter("%(message)s"))
        # TODO
        ...
        self._wb_header += [item[1]['name']
                            for item in self.exchanges if item[0] == 'exchange']
        self.wb_logger.info(', '.join(self._wb_header))
        # combined model time
        self._dt = timedelta(seconds=int(
            self._config.get('coupling', 'dt', fallback=86400)))
        # set start_time & end_time if given
        if self._config.has_option('coupling', 'start_time'):
            start_time = datetime.strptime(self._config.get(
                'coupling', 'start_time'), "%Y-%m-%d")
            self.set_start_time(start_time)
        if self._config.has_option('coupling', 'end_time'):
            end_time = datetime.strptime(self._config.get(
                'coupling', 'end_time'), "%Y-%m-%d")
        ...
        for ex_i, ex_from in enumerate(self._config.options('exchanges')):
            ...
            for i, var_from in enumerate(vars_from):
                ...
                if isinstance(var_from, str) and var_from not in from_model_vars:
                    self.logger.warning(
                        "Unkonwn variable {} for model {}".format(var_from, from_bmi._name))
            # TO
            # index
            ...
            for i, var_to in enumerate(vars_to):
                ...
                if isinstance(var_to, str) and var_to not in to_model_vars:
                    self.logger.Warning(
                        "Unkonwn variable {} for model {}".format(var_to, to_bmi._name))
            ...
            exdict['name'] = '{:s}.{:s}_to_{:s}.{:s}'.format(
                mod_from, exdict['from_vars'][0], mod_to, exdict['to_vars'][0])
            # UPDATE
    ...
    def _set_spatial_coupling(self):
        ...
        for item in self.exchanges:
            if item[0] == 'exchange':
                self.logger.info('set coupled grids for: {}'.format(
                    str(item[1]['coupling'])))
    ...
    def update(self, dt=None):
        ...
        for item in self.exchanges:
            if item[0] == 'update':
                ...
                tot_volume_in = getattr(
                    self.bmimodels[imod], '_get_tot_volume_in', lambda: -9999.)()
                wb_dict.update(
                    {'{}_tot_in'.format(imod): '{:.2f}'.format(tot_volume_in)})
                tot_volume_out = getattr(
                    self.bmimodels[imod], '_get_tot_volume_out', lambda: -9999.)()
                wb_dict.update({'{}_tot_out'.format(
                    imod): '{:.2f}'.format(tot_volume_out)})
    ...
    def exchange(self, from_mod, to_mod, from_vars, to_vars, coupling, add=False, **kwargs):
        ...
        self.logger.info('{} {}.{} data to coupled {}.{} variable'.format(
            'adding' if add else 'setting', from_mod, from_vars[0], to_mod, to_vars[0]))
        if coupling.at_indices():
            tot_volume = self.exchange_at_indices(
                from_mod, to_mod, from_vars, to_vars, coupling, add=add, **kwargs)
        else:
            tot_volume = self.exchange_same_grid(
                from_mod, to_mod, from_vars, to_vars, add=add, **kwargs)
        return tot_volume
    ...
    def exchange_same_grid(self, from_mod, to_mod, from_vars, to_vars, add=False, **kwargs):
        ...
        if (from_mod == 'PCR') and (from_vars[0] == 'runoff') and (to_mod == 'CMF') and (to_vars[0] == 'roffin'):
            tot_volume = np.nansum(
                vals * self.bmimodels[from_mod].get_value(self.bmimodels[from_mod]._area_var_name))
        ...
        for var in to_vars[1:]:
            ...
            assert np.all(np.not_equal(div, 0))
            vals /= div
        # SET data
    ...
    def exchange_at_indices(self, from_mod, to_mod, from_vars, to_vars, coupling, add=False, **kwargs):
        ...
        for from_var in from_vars:
            if isinstance(from_var, float):  # scalar multiplier
                ...
                vals_get *= self.bmimodels[from_mod].get_value_at_indices(
                    from_var, coupling.from_ind)
        ...
        if coupling.to_grp_n.max() > 1:  # split using fractions
            vals_set = np.repeat(
                vals_get, coupling.to_grp_n) * coupling.get_frac()
        else:
            vals_set = vals_get
        ...
        if vol_diff > 1E-2:
            self.logger.warming(
                'Large difference in water volume from and to: {:.4f} m3'.format(vol_diff))
        ...
        for var in to_vars[1:]:
            if isinstance(var, float):
                ...
                div = self.bmimodels[to_mod].get_value_at_indices(
                    var, coupling.to_ind)
            # make sure not to divide by zero
            assert np.all(np.not_equal(div, 0))
            vals_set /= div
        # SET data
        if add:  # add to current state
            vals_set += self.bmimodels[to_mod].get_value_at_indices(
                to_vars[0], coupling.to_ind)
        self.bmimodels[to_mod].set_value_at_indices(
            to_vars[0], coupling.to_ind, vals_set)
        return tot_volume
    ...
    def get_start_time(self):
        ...
        self._startTime = max([self.bmimodels[mod].get_start_time()
                               for mod in self.bmimodels])
    ...
    def get_current_time(self):
        ...
        in_sync = not models_t or models_t.count(
            models_t[0]) == len(models_t)  # check if identical
        if not in_sync:
            msg = '; '.join(['{}: {}'.format(m, str(t))
                             for m, t in zip(self.bmimodels, models_t)])
    ...
    def get_end_time(self):
        ...
        self._endTime = min([self.bmimodels[mod].get_end_time()
                             for mod in self.bmimodels])
