...
class CMF(GBmi):
    ...
    def initialize_config(self, config_fn):
        ...
        self._config = glib.configread(
            self._config_fn, encoding='utf-8', cf=NamConfigParser())
        # model time
    ...
    def update(self, dt=None):
        ...
        while self._t < t_next:
            ...
            i += 1
        self.logger.info('updated model to datetime {} in {:d} iterations'.format(
    ...
    def set_value(self, long_var_name, src, **kwargs):
        ...
        src = np.where(np.isnan(src), 0, src).astype(
    ...
    def _get_tot_volume_in(self):
        # runoff in [m3/s]
        Rin = np.nansum(self.get_value('runoff')) * \
            float(self.get_attribute_value('CONF:DTIN'))
        return Rin

    def _get_tot_volume_out(self):
        Qout = np.nansum(self.get_value_at_indices(
            'outflw', self.grid.pits)) * self._dt.total_seconds()
        return Qout
    ...
    def get_grid(self, fn_params=r'params.txt', fn_locs=r'./hires/location.txt', **kwargs):
        if not hasattr(self, 'grid') or (self.grid is None):
            ...
            self.logger.info(
                'Getting Unit-Catchment Grid info based on {}'.format(basename(fn_params)))
            p = read_params(fn_params)
            # get hires index info file
            fn_locs = join(self._mapdir, fn_locs)
            if not isfile(fn_locs):
                raise IOError(
                    "locations.txt file for hires index not found {}".format(fn_locs))

            # get catchment unit grid instance
            # TODO: set crs
            self.grid = UCGrid(p['transform'], p['height'],
                               p['width'], fn_locs)
            ...
            if not isfile(fn_locs):
                raise IOError(
                    "nextxy file for drainage direction data not found {}".format(fn_locs))
            self.logger.info(
                'Getting drainage direction from {}'.format(basename(fn_nextxy)))
            ...
            if isfile(latlon_fn):
                ...
                lonlat = np.fromfile(latlon_fn, dtype=np.float32).reshape(
                    2, self.grid.height, self.grid.width)  # read lat lon unit catchment outlets
                ...
                # 2d index of inds
                id2 = np.ones(self.grid.shape, dtype=np.int32)
                id2[row, col] = i
                ...
                self.grid.set_1d(nodes=nodes, links=links, inds=inds,
                                 ds_idx=next_inds, us_idx=inds[valid])
            else:
                Warning(
    ...
    def set_attribute_value(self, attribute_name, attribute_value):
        glib.configcheck(self, self.logger)
        self.logger.debug(
            "set_attribute_value: {} -> {}".format(attribute_name, attribute_value))
    ...
    def set_inpmat(self, bounds, res, olat='NtoS'):
        ...
        if not isfile(join(self._mapdir, 'generate_inpmat')):
            raise ValueError('{} not found'.format(
                join(self._mapdir, 'generate_inpmat')))
        w, s, e, n = bounds
        # generate inpmat
    ...
    def set_inpmat_attrs(self):
        ...
        self.set_attribute_value(
            'INPUT:CINPMAT', '"{:s}/inpmat-tmp.bin"'.format(rel_path))
        self.set_attribute_value('INPUT:LBMIROF', ".TRUE.")
        self.set_attribute_value(
            'MAP:CDIMINFO', '"{:s}/diminfo_tmp.txt"'.format(rel_path))
...
class NamConfigParser(ConfigParser):
    ...
    def write(self, fp, space_around_delimiters=False):
        ...
        """
        super(NamConfigParser, self).write(
