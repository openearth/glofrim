...
class LFP(GBmi):
    ...
    def initialize_config(self, config_fn, config_defaults={}):
        ...
        defaults = {'refdate': '2000-01-01'}
        defaults.update(**config_defaults)
        self._config_fn = abspath(config_fn)
        self._config = glib.configread(
            self._config_fn, encoding='utf-8', cf=ParConfigParser(defaults=defaults))
        self._datefmt = "%Y-%m-%d"
        # model time
        ...
        self._mapdir = dirname(glib.getabspath(
            str(self.get_attribute_value('DEMfile')), _root))
    ...
    def get_grid(self):
        if not hasattr(self, 'grid') or (self.grid is None):
            ...
            _dem_fn = glib.getabspath(
                str(self.get_attribute_value('DEMfile')), self._mapdir)
            if not isfile(_dem_fn):
                raise IOError('DEMfile file not found')
            self.logger.info(
                'Getting rgrid info based on {}'.format(basename(_dem_fn)))
            with rasterio.open(_dem_fn, 'r') as ds:
                self.grid = RGrid(ds.transform, ds.height, ds.width,
                                  crs=ds.crs, mask=ds.read(1) != ds.nodata)
            # riv width file used for "1D coords"
            _width_fn = glib.getabspath(
                str(self.get_attribute_value('SGCwidth')), self._mapdir)
            if not isfile(_width_fn):
                raise IOError('SGCwidth file not found')
        ...
        if isinstance(start_time, datetime):
            ...
            try:
                ...
                start_time = datetime.strptime(
                    start_time, self._datefmt)  # check format
            except ValueError:
    ...
    def set_end_time(self, end_time):
        ...
        TStop = (end_time - refdate).seconds + \
            (end_time - refdate).days * 86400
        TStop = '{:.0f}'.format(TStop)
        self._endTime = end_time
        self.set_attribute_value('sim_time', TStop)

    def set_out_dir(self, out_dir):
        self.set_attribute_value('dirroot', relpath(
    ...
    def write_config(self):
        ...
        self._config_fn = glib.write_config(
            self, self._config, self._config_fn, self.logger)
...
class ParConfigParser(ConfigParser):
    def __init__(self, **kwargs):
        # keep format with capital/lower letters
