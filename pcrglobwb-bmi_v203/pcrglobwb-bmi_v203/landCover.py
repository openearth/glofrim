...
class LandCover(object):
    ...
    def __init__(self,iniItems,nameOfSectionInIniFile,parameters,landmask,usingAllocSegments = False):
        ...
        self.ActivateZeroInfiltrationFloodplain   = iniItems.globalOptions['ActivateZeroInfiltrationFloodplain']
        self.ActivateFactorInfiltrationFloodplain = iniItems.globalOptions['ActivateFactorInfiltrationFloodplain']
        
        # introduce relevant variables for 2-way coupling so they can be changed through BMI when needed
        self.floodplainWaterLayer    = pcr.ifthen(self.landmask, pcr.scalar(0.0))  # layer of water from D-Flow FM
        self.inundatedFraction       = pcr.ifthen(self.landmask, pcr.scalar(0.0))  # fraction of inundated area from D-Flow FM
        self.controlFloodplainFactor = pcr.ifthen(self.landmask, pcr.scalar(1.0))  # to control where the floodplain infiltration scaling factor is applied
        
        # introduce variables used in 2-way coupling calculations so they can be called with BMI's 'get_var'
        self.directRunoffFloodplain  = pcr.spatial(pcr.scalar(0.))    # direct runoff to be calculated from floodplainWaterLayer
        self.openWaterEvapFloodplain = pcr.spatial(pcr.scalar(0.))    # evaporation to be calculated from floodplainWaterLayer
        self.infiltrationFloodplain  = pcr.spatial(pcr.scalar(0.))    # infiltration to be calculated from floodplainWaterLayer
        # ----------------------------------------------------------------------------------------------------------------
    ...
    def calculateDirectRunoff(self, parameters):
        ...
        # ADDED: perform adjusted calculations if ActivateCoupling is set to True
        # ----------------------------------------------------------------------------------------------------------------
        if self.ActivateCoupling == 'True':
            
            # scale directRunoff from topWaterLayer based on non-inundated fraction
            self.directRunoff = self.directRunoff * pcr.max(0., (1 - self.inundatedFraction))
            
            # update floodplainWaterLayer
            # with netLqWaterToSoil and irrGrossDemand
            self.floodplainWaterLayer = self.floodplainWaterLayer + \
                                        pcr.max(0.,self.netLqWaterToSoil + self.irrGrossDemand)
            
            # floodplainWaterLayer is partitioned into directRunoff (and infiltration)
            self.directRunoffFloodplain = self.improvedArnoScheme(\
                                          iniWaterStorage = self.soilWaterStorage,\
                                          inputNetLqWaterToSoil =  self.floodplainWaterLayer, 
                                          parameters = parameters)
            self.directRunoffFloodplain = pcr.min(self.floodplainWaterLayer, self.directRunoffFloodplain)
            # Yet, no directRunoff in the paddy field.
            if self.name == 'irrPaddy': self.directRunoff = 0.
            
            # update floodplainWaterLayer after directRunoff
            self.floodplainWaterLayer = self.floodplainWaterLayer - self.directRunoffFloodplain
            
            # scale directRunoff from floodplainWaterlayer based on inundated fraction
            self.directRunoffFloodplain = self.directRunoffFloodplain * (self.inundatedFraction)
            
            # add the directRunoff values based on fractions to obtain total directRunoff
            self.directRunoff = self.directRunoff + self.directRunoffFloodplain
        # ----------------------------------------------------------------------------------------------------------------
    ...
    def calculateOpenWaterEvap(self):
        ...
        if self.ActivateCoupling == 'True':
            ...
            # scale openWaterEvap from topWaterLayer based on non-inundated fraction
            self.openWaterEvap = self.openWaterEvap * pcr.max(0., (1 - self.inundatedFraction))
            ...
            self.openWaterEvapFloodplain = pcr.spatial(pcr.scalar(0.))
            remainingPotETP = self.potBareSoilEvap + self.potTranspiration
            # remainingPotETP = self.totalPotET
            
            # open water evaporation from floodplains (not limited to 'irrPaddy')
            self.openWaterEvapFloodplain =  \
             pcr.min(\
             pcr.max(0.,self.floodplainWaterLayer), remainingPotETP)
            
            # update floodplainWaterLayer layer after openWaterEvap of floodplains
            self.floodplainWaterLayer = pcr.max(0.,self.floodplainWaterLayer - self.openWaterEvapFloodplain)
            
            # scale openWaterEvap from floodplainWaterlayer based on inundated fraction
            self.openWaterEvapFloodplain = self.openWaterEvapFloodplain * (self.inundatedFraction)
            
            # update potBareSoilEvap & potTranspiration (after openWaterEvap of floodplains)
            self.potBareSoilEvap  =       pcr.cover( self.potBareSoilEvap -\
                                   (self.potBareSoilEvap/remainingPotETP)*
                                    self.openWaterEvapFloodplain, 0.0)       
            self.potTranspiration =       pcr.cover( self.potTranspiration-\
                                  (self.potTranspiration/remainingPotETP)*
                                    self.openWaterEvapFloodplain, 0.0)
            
            # add the openWaterEvap values based on fractions to obtain total openWaterEvap
            self.openWaterEvap = self.openWaterEvap + self.openWaterEvapFloodplain
    ...
    def calculateInfiltration(self, parameters):
        ...
        if self.ActivateCoupling == 'True':
            ...
            if self.ActivateZeroInfiltrationFloodplain == 'False':
                
                # scale infiltration from topWaterLayer based on non-inundated fraction
                self.infiltration = self.infiltration * pcr.max(0., (1 - self.inundatedFraction))
                
                # Infiltration from floodplains, limited with KSat1 and available water in floodplainWaterLayer:
                
                # check if kSat scaling factor is to be used
                if self.ActivateFactorInfiltrationFloodplain == 'True':
                    # calculate scaling factor based on saturated area fraction
                    self.factorInfiltrationFloodplain = pcr.max(pcr.max(0., (1 - self.satAreaFrac)), self.controlFloodplainFactor)
                    # calculate infiltration
                    if self.numberOfLayers == 2:
                        self.infiltrationFloodplain = pcr.min(self.floodplainWaterLayer, parameters.kSatUpp * self.factorInfiltrationFloodplain)
                    if self.numberOfLayers == 3:
                        self.infiltrationFloodplain = pcr.min(self.floodplainWaterLayer,parameters.kSatUpp000005 * self.factorInfiltrationFloodplain)
                else:
                    if self.numberOfLayers == 2:
                        self.infiltrationFloodplain = pcr.min(self.floodplainWaterLayer, parameters.kSatUpp)
                    if self.numberOfLayers == 3:
                        self.infiltrationFloodplain = pcr.min(self.floodplainWaterLayer,parameters.kSatUpp000005)
                
                # update floodplainWaterLayer layer after infiltration
                self.floodplainWaterLayer = pcr.max(0.,self.floodplainWaterLayer - self.infiltrationFloodplain)
                
                # scale infiltration from floodplainWaterlayer based on inundated fraction
                self.infiltrationFloodplain = self.infiltrationFloodplain * (self.inundatedFraction)
                
                # add the infiltration values based on fractions to obtain total infiltration
                self.infiltration = self.infiltration + self.infiltrationFloodplain
                
                # add the remaining water back to the general topWaterLayer using inundation fractions
                self.topWaterLayer = self.topWaterLayer * pcr.max(0., (1 - self.inundatedFraction)) + self.floodplainWaterLayer * self.inundatedFraction
    
                # and reset floodplainWaterLayer to zero (for proper water balance checks)
                self.floodplainWaterLayer = pcr.ifthen(self.landmask, pcr.scalar(0.0))
            
            elif self.ActivateZeroInfiltrationFloodplain == 'True':
                
                # add floodplainWaterLayer back to the general topWaterLayer without calculating infiltration
                self.topWaterLayer = self.topWaterLayer + self.floodplainWaterLayer * self.inundatedFraction
    ...
    def upperSoilUpdate(self,meteo,groundwater,routing,\
        ...
        if self.debugWaterBalance == str('True'):
            ...
            # CHANGED: added preFloodplainWaterLayer (over the entire cell) to make sure water balance checks go correctly
            # NOTE: this will make sure it goes correctly here, but it does not influence the check in the pcrglobwb module!
            # ----------------------------------------------------------------------------------------------------------------
            if self.ActivateCoupling == 'True':
                preTopWaterLayer = self.topWaterLayer + (self.floodplainWaterLayer * self.inundatedFraction)
            else:
